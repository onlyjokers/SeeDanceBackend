import { retryOperation, type RetryEvent } from "./retry.js";
...
export async function uploadImageToTemporaryHost(
  ...
  const run = async () => {
    ...
    return {
      provider: "uguu" as const,
      url,
      expiresIn: "temporary" as const
    };
  };
  return retryOptions ? retryOperation(run, retryOptions) : run();
}
